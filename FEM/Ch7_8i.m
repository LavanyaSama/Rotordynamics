...
d = 0.03;   % diameters of shafts in m
    ...
    connect = [1 1 2;   %% First Column is element number
     ...
     Ip=0.02;     % mass of disc at each node
...
for i = 1:nele              %loop for finding elemental mass &
    ...
    kele(:,:,i) = G*J/l*[1 -1;-1 1];
     if(i<nele)
        mele(:,:,i) =(rho*J*l/6)*[2 1; 1 2];      %disc is included only in right side
     else
        mele(:,:,i) =[0 0;0 Ip]+(rho*J*l/6)*[2 1; 1 2];
     end
...
disp('Solution is printed to a text file "Exercise7.8_output_FEM.txt"');
disp('mode shape is saved as "Mode_shape_Exercise7.8_FEM.fig"');
...
ylim([-1.2 1.2]);
legend('show');
saveas(h,'Exercise7.8_mode_shape_FEM','png');
%----------------------------------------------------------------------
